...
# Data source: KEYNOTE-024 (Reck M, et al. N Engl J Med. 2016;375:1823-33),
#              Figure 2 (Overall Survival), digitized via WebPlotDigitizer.
...
# SECTION 2 — Guyot (2012) pseudo-IPD reconstruction

# Reconstructs pseudo individual patient data (IPD) from a digitized
# Kaplan-Meier curve, following the method of:
#   Guyot P, Ades AE, Ouwens MJ, Welton NJ. "Enhancing secondary analysis of

# Implemented from scratch in base R (no CRAN packages such as IPDfromKM or
# flexsurv are available in this environment), so every step is explicit and
# auditable rather than hidden inside a package call.
#
# Core idea:
#   The published risk table gives us "ground truth" checkpoints -- we know
#   exactly how many patients were still at risk at t = 0, 3, 6, ... months.
#   Between two checkpoints, the *shape* of the digitized curve tells us how
#   the risk set most likely thinned out (via the standard KM step formula
#   S(t_i)/S(t_i-1) = (r_i - d_i) / r_i). Any gap left between what the curve
#   implies and what the risk table actually reports is attributed to
#   censoring, and spread across the interval rather than dumped at one point.
# ============================================================================

#' Reconstruct pseudo-IPD for a single treatment arm
#'
...
# ============================================================================
# Run reconstruction for both trial arms and save the combined pseudo-IPD
# ============================================================================

# ============================================================================
# SECTION 3 — Parametric survival model fitting  (was: R/02_parametric_...)
# ============================================================================
# ============================================================================
# 02_parametric_models.R
#
# Fits standard parametric survival distributions to right-censored
# time-to-event data by direct maximum likelihood estimation.
#
# Note on package choice: the obvious tool for this (flexsurv) is not
# installed and this sandbox has no route to CRAN to fetch it, so every
# distribution's log-likelihood is written out explicitly below and
# optimized with base R's optim(). This is the same approach flexsurv uses
# internally -- it's just done by hand here, which also makes every
# assumption inspectable rather than hidden in a package.
#
# Distributions included:
#   - Exponential, Weibull, Gamma      (the three the project asked for)
#   - Log-normal, Log-logistic, Gompertz (added because these six together
#     are the standard extrapolation candidate set recommended by NICE DSU
#     TSD 14 for survival extrapolation in health economic models -- a
#     reviewer at a HEOR shop will recognize this set immediately)
# ============================================================================

# ---- distribution definitions: log-density and log-survival ---------------
# Each entry gives: number of parameters, a function mapping optim's
# unconstrained parameter vector -> interpretable parameters, and the
# log f(t) / log S(t) functions needed for the likelihood.
...
# ============================================================================
# ---- Reconstruct pseudo-IPD once at startup (fast: <1s) -------------------
...
server <- function(input, output, session) {
  ...
  output$dl_ipd <- downloadHandler(
    ...
    content  = function(file) write.csv(ipd_a
                                        ll, file, row.names = FALSE)
  )
}
